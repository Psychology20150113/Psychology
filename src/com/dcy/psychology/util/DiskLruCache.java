...
public class DiskLruCache {
	...
	private int maxCacheItemSize = 64; // 64 item default
	 ...
	 * @return
	 */
	public static DiskLruCache openCache(Context context, File cacheDir, long maxByteSize, int count) {
		if (!cacheDir.exists()) {
			cacheDir.mkdir();
		}

		if (cacheDir.isDirectory() && cacheDir.canWrite()) {
			return new DiskLruCache(cacheDir, maxByteSize, count);
		}
	 ...
	 * @param maxByteSize
	 */
	private DiskLruCache(File cacheDir, long maxByteSize, int count) {
		mCacheDir = cacheDir;
		maxCacheByteSize = maxByteSize;
		maxCacheItemSize = count;
	}
